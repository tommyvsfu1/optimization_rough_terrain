	...
	void	cloudCB(const	sensor_msgs::PointCloud2	&input)
	{
		...
		for (double distance = Kinect_view_Front_cutoff+0.9; distance < Kinect_view_Rear_cutoff; distance += Define_grid) {
			...
			for (int j = 0; j < NumOFPatch; j++) {
				ID[i][j].width = tmp.width;
				ID[i][j].height = tmp.height;
				ID[i][j].is_dense = tmp.is_dense;
				ID[i][j].points.resize( ID[i][j].width * ID[i][j].height );
			
				// filter
				for (size_t k = 0; k < tmp.points.size(); ++k) {
					//std::cout << "distance " <<  distance << std::endl;
					if (tmp.points[k].z >= distance-0.9 && tmp.points[k].z <= distance) {
						...
						std::cout << (0.9*( -(NumOFPatch/2) +j)) << " " << (0.9*( -(NumOFPatch/2) +j+1 ) ) << std::endl;
						 if (tmp.points[k].x > (0.9*( -(NumOFPatch/2) +j ) ) && tmp.points[k].x < (0.9*( -(NumOFPatch/2) +j+1 ) ) ) {
								std::cout << "fuck" << std::endl;
	     	      						ID[i][j].points[k].x = tmp.points[k].x;
	     	      						ID[i][j].points[k].y = tmp.points[k].y;
	     	      						ID[i][j].points[k].z = tmp.points[k].z;
	     	      						ID[i][j].points[k].rgb = tmp.points[k].rgb;		
